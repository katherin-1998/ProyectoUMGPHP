...
<div class="rep-wrapper">
    <div class="rep-card">
        <div class="rep-header">
            ...
            <p class="rep-title">Reporte de Ingresos por Salón</p>
        </div>
        <p class="rep-subtitle">
            Muestra los registros de ingreso filtrados por <strong style="color:#1a4a8a">sede, salón y fecha</strong>.
        </p>
        ...
        <form id="formSalon">
            <div class="row g-3 mb-3">
                <div class="col-md-3">
                    ...
                    <select name="sede" id="sede_salon" class="rep-select">
                ...
                <div class="col-md-3">
                    ...
                    <div class="rep-label">Salón</div>
                    <select name="salon" id="salon_salon" class="rep-select" disabled>
        ...
        <div id="tablaSalon">
            <p class="loading-box">Seleccione sede, salón y fecha para ver los ingresos.</p>
        </div>
    </div>
</div>

<script>
// Cargar salones al cambiar sede
document.getElementById("sede_salon").addEventListener("change", function() {
    const salonSel = document.getElementById("salon_salon");
    salonSel.innerHTML = '<option value="">— Seleccione Salón —</option>';
    salonSel.disabled = true;
    if (!this.value) return;

    fetch("/ProyectoUMGPHP/php/reporte_salon/php/get_salones_por_sede.php?id_sede=" + this.value)
        .then(r => r.json())
        .then(data => {
            if (data.length > 0) {
                salonSel.disabled = false;
                data.forEach(s => {
                    salonSel.innerHTML += `<option value="${s.id_salon}">${s.nombre_salon}</option>`;
                });
            }
        })
        .catch(() => {
            // Si no existe ese endpoint, simplemente habilitamos el select vacío
            salonSel.disabled = false;
        });
});
...
document.getElementById("formSalon").addEventListener("submit", function(e) {
    ...
    document.getElementById("tablaSalon").innerHTML = '<div class="loading-box">Cargando datos...</div>';
    fetch("ingreso_salon_ajax.php", { method: "POST", body: formData })
        .then(res => res.text())
